...
// and renders agent activity in real time. This is the visual centerpiece
// of every demo moment — the reasoning trace IS the product.
...
export function AgentTrace({ prompt, sessionId, city, onComplete }: AgentTraceProps) {
  ...
  function handleEvent(event: AgentEvent) {
    switch (event.type) {
      ...
      case 'done':
        // Stream closing — leave final states intact
        break;
    }
  }

  // Auto-scroll trace pane to bottom
  useEffect(() => {
    ...
    case 'extracted':
      return (
        <div className="flex justify-between items-baseline border-l-2 border-blue-300 pl-2">
          <span className="text-neutral-900">📋 {trace.title}</span>
      ...
      return (
        <div className="bg-green-50 rounded px-3 py-2">
          <div className="flex items-baseline justify-between mb-1">
            <span className="text-green-900 font-medium">🔗 Merged: {trace.title}</span>
    ...
    case 'recommendation':
      return (
        <div className="border-l-2 border-amber-300 pl-2">
          <div className="text-xs text-amber-700">★ {(trace.score * 100).toFixed(0)}%</div>
          <div className="text-neutral-700 text-sm">{trace.reasoning}</div>
        </div>
      );
    case 'error':
      return (
        <div className="text-red-700 text-sm bg-red-50 rounded px-2 py-1">⚠ {trace.message}</div>
      );
  }
}
